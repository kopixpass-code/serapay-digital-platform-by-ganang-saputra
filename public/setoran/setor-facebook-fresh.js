...
const validEmail =
/^[A-Z0-9._%+-]+@(gmail\.com|gaslur\.com|bosgb\.com)$/i
.test(email);

const isGmail =
nama.toLowerCase().includes("gmail");

const validGmailEmail =
...
const validLink =
/^https:\/\/www\.facebook\.com\/profile\.php\?id=6158\d{10}$/i
.test(secondRow);

if(isGmail && !validGmailEmail){

    toast(
        `Akun No-${Math.floor(i/step)+1} bukan akun gmail`
    );
...
if(isFacebook){
    ...
    if(!validEmail){
            ...
            toast(
    `Akun ke-${Math.floor(i/step)+1} tidak valid, kami hanya menerima akun yang dibuat dengan domain @gmail.com, @gaslur.com atau @bosgb.com` );
...
"loadingInfo"
    ...
    if(!validUID && !validLink){

        toast(
            `Akun ke-${Math.floor(i/step)+1} harus berupa UID 14 digit atau link profile Facebook yang valid`
        );
...
"loadingInfo"
    ...
    if(uid === "-"){
        ...
        toast(
            `Akun ke-${Math.floor(i/step)+1} UID Facebook tidak ditemukan`
        );
...
toast(
"Menunggu ditinjau, silahkan cek riwayat"
);
...
<td>${x.password || x.Password || "-"}</td>

<td>
<span class="badge ${cls}">
${x.status_akun || x.Status || "Pending"}
</span>
</td>

<td>${x.produk || x.Produk || "-"}</td>
